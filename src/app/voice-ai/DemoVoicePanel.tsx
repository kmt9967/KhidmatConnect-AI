...
/**
 * DemoVoicePanel — M17 SECONDARY, clearly-labelled DEMO fallback for /voice-ai.
 *
 * This preserves the original Milestone-9 simulated experience (mock transcript,
 * mock case draft, demo controls) for offline illustration and judging when a
 * live browser call is not possible. It is DEMOTED: it is never the default, it
 * is wrapped in an explicit DEMO MODE banner, and every invented field (caller
 * number, cell tower, fixed case id, ASR latency) is tagged DEMO so it can never
 * be mistaken for a live emergency. The REAL experience lives in
 * RealVoicePanel.tsx. No demo value here is ever submitted to the backend.
 */
...
import {
  Bot, Check, ChevronDown, ChevronRight, Clock, FileText, Info, Mic, Pause,
  Phone, PhoneCall, PhoneOff, Play, AlertOctagon, AlertTriangle, ExternalLink,
  MapPin, ShieldCheck, Sparkles, Terminal, User, Volume2, RotateCcw,
} from 'lucide-react';
...
export default function DemoVoicePanel({ t, isUrdu }: Props) {
  // Call lifecycle (all simulated)
  ...
  const [audioPlaybackProgress] = useState(65);
  ...
  // Voice state from turn (simulated)
  useEffect(() => {
    if (callPhase !== 'active') return;
  ...
  }, [currentTurnIndex, callPhase, conversationTurns]);
  ...
  // ─── Demo Actions (simulate ONLY; never touch the backend) ──
  ...
  const DemoTag = () => (
    <span className="text-[9px] font-mono font-bold text-gray-500 bg-[#0B0E14] px-1 py-0.5 rounded border border-[#30363D] uppercase">Demo</span>
  );

  return (
    <>
      {/* ═══ DEMO MODE BANNER ═══ */}
      <div className="bg-amber-950/40 border-b border-amber-500/40 px-4 sm:px-6 py-2 flex items-center gap-2 text-[11px] font-bold text-amber-300">
        <Info className="w-4 h-4 shrink-0" />
        <span>{t.voiceDemoModeBanner}</span>
      </div>

      {/* ═══ CALL METADATA STRIP (simulated — tagged DEMO) ═══ */}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-5 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6">
        {/* ─── LEFT: Simulated Call & Transcript ─── */}
        ...
        {/* ─── RIGHT: Simulated Case Extraction ─── */}
            ...
            <div className="space-y-2.5 text-xs">
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 rounded-2xl bg-[#0B0E14] border border-[#30363D] space-y-0.5">
                  ...
                  <p className="font-mono text-sm font-black text-white flex items-center gap-1.5">{extractedCase.id} <DemoTag /></p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="p-3 rounded-2xl bg-[#0B0E14] border border-[#30363D] space-y-0.5">
                  ...
                  <p className="font-mono text-xs font-bold text-emerald-400 flex items-center gap-1.5" dir="ltr">{extractedCase.contact} <DemoTag /></p>
                </div>
              </div>
