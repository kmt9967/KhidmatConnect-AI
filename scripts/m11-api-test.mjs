...
async function main() {
  ...
  // 2. GET case detail (KC-2026-000001)
  console.log('\n2. GET case detail as OPERATOR');
  const caseRes = await authedFetch(
    `${BASE}/api/operator/cases/KC-2026-000001`,
    {},
    opLogin.cookies
  );
  const caseData = await caseRes.json();
  assert('Case detail returns 200', caseRes.status === 200, `status=${caseRes.status}`);
  assert('Case has caseCode', caseData.caseCode === 'KC-2026-000001', caseData.caseCode);
  ...
  const resRes = await authedFetch(
    `${BASE}/api/operator/cases/KC-2026-000001/assign`,
    {},
    opLogin.cookies
  );
  ...
  const noteRes = await authedFetch(
    `${BASE}/api/operator/cases/KC-2026-000001/notes`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const noAuthRes = await fetch(`${BASE}/api/operator/cases/KC-2026-000001/notes`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const noAuthCaseRes = await fetch(`${BASE}/api/operator/cases/KC-2026-000001`);
